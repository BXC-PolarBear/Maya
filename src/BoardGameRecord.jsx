...
export default function BoardGameRecord({ user, activeGameRoom, onBack }) {
      ...
      const fetchMyData = async () => {
        try {
          ...
          if (snap.exists()) {
            ...
            if (data.summary && data.summary.activeRound) setActiveRound(data.summary.activeRound);
          }
  ...
  const handleEditFootprint = (footprint) => {
    ...
    setCardType(raw.cardType || 'direct'); setDirectScore(raw.directScore || '');
    setDice1(raw.dice1 || 1); setDice2(raw.dice2 || 1); setFixedType(raw.fixedType || '5');
  };
  ...
  const handleAddFootprint = () => {
    ...
    const footprintData = {
      diceSteps: parseInt(diceSteps) || 0, fastForward: parseInt(fastForward) || 0,
      color: selectedColor, colorName: cardColors.find(c => c.id === selectedColor).name,
      score, detail, rawInput: { cardType, directScore, dice1, dice2, fixedType }
    };
    ...
    <div style={{ width: '100%' }}>
      ...
      {isHost && isHostPlaying && (
        <div style={{ display: 'flex', width: '100%', marginBottom: '15px', backgroundColor: '#fff', borderRadius: '12px', padding: '4px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          ...
          <button onClick={() => setActiveTab('player_list')} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', backgroundColor: activeTab === 'player_list' ? '#e3f2fd' : 'transparent', color: activeTab === 'player_list' ? '#1976d2' : '#888', fontWeight: 'bold', cursor: 'pointer' }}>
            👥 玩家列表
          </button>
        </div>
      )}
      ...
      <>
        ...
        <div style={blockStyle}>
          ...
          <div style={{ display: 'flex', gap: '6px', fontSize: '11px', color: '#888', textAlign: 'center', marginBottom: '6px', paddingLeft: '46px' }}>
            ...
            <div style={{ flex: 1 }}>流年 Age</div><div style={{ flex: 1 }}>流年 KIN</div>
          </div>
